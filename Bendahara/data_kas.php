...
<html lang="en">
  ...
  <?php include "form_kas.php"; ?>

  <div class="col-8" style="border: 1px solid lightgray; border-radius: 10px; padding: 10px;">
  <h3 style="text-align: center; background-color: lightblue; border-radius: 10px; color: white; padding: 10px;">Data Kas</h3>

<a href = "laporan_kas.php" target="_blank" class="btn btn-success
            mb-2 mt-1">Cetak Data</a>

  <form action = "data_kas.php" method = "POST">
  ...
  <tbody>
    ...
    foreach($data_kas as $row) {
      ...
      <tr>
        ...
        <td><?php echo $row['kd_kas'] ?></td>
        <td><?php echo $row['nm_kas'] ?></td>
        <td><?php echo $row['saldo'] ?></td>
        <td><?php echo $row['nm_pj'] ?></td>
        <td align="center" width="150px">
        <a class="btn btn-sm btn-warning" href="data_kas.php?&edit=update&&id=<?= $row['kd_kas'] ?>">Edit</a>
                                    <a class="btn btn-sm btn-danger" href="proses_kas.php?aksi=delete&&id=<?= $row['kd_kas'] ?>" onclick="javascript: return confirm('Apakah yakin data dihapus')">Delete</a>
          </td>
      </tr>
      <?php
    }
